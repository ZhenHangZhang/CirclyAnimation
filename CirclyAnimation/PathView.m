...
- (void)gradentWith:(CGRect)frame{
    ...
    UIBezierPath *path = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.bounds.size.width/2, self.bounds.size.height/2) radius:(frame.size.width-PROGRESS_LINE_WIDTH)/2-5 startAngle:degressToRadius(-270) endAngle:degressToRadius(-271) clockwise:YES];
    //根据原来的frame 进行进度渐变
    ...
    replica.frame = inset;
    replica.backgroundColor = [UIColor whiteColor].CGColor;
